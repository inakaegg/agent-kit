...
- PRを作成したら、その場で `$pr-review-loop` に入り、最新headに対するbot review・CI・required checksを収束条件まで回す（botが利用上限等で止まっている場合の扱いも同Skillに従う）。mergeは§3のとおり明示許可を必要とする（`AUTO_MERGE_PRIVATE=true` の非公開・非共有repositoryでは、必要なレビューがすべてLGTMなら許可不要）。
