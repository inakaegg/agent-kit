...
5. **最小構成を選び計画する** — OS標準、成熟したライブラリ、既存API・規格、既存処理の単純な組合せを先に評価する。利用者価値が同等なら、規則・状態・依存が少ない案を選ぶ。選んだ方式、変更対象、検証手順を実装計画としてactive planへ書く。§7の重リスク作業では、実装へ着手する前に計画レビューを通す。
