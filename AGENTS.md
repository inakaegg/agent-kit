...
- 人間向け文書の新規作成と本文の実質更新は、`REVIEW_MODEL_READABILITY` が示す担当の、履歴を共有しない別セッションによる読みやすさレビューを通してからcommitする（`READABILITY_REVIEW=false` または `DOCS_LEVEL=minimal` なら不要）。誤字修正等の軽微変更は対象外。
