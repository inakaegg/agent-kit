...
- PR作成の直後に `$pr-review-loop` へ入り、最新headのbot review・CIを確認する。botが利用上限等で止まっていれば、その事実と再開時刻を同Skillのstate記録へ残す。再依頼のcomment投稿はremote writeなので §3 の許可範囲に従い、許可がなければ再開時刻の記録と報告に留める。
