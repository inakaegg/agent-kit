...
- commit messageはrepository規則（明文か、履歴の支配的な形式）を優先する。規則がなければ、件名は「**英語の要約 / 日本語の要約**」を斜線で並べた1行とする（`COMMIT_SUBJECT_LANG_ORDER`。`en-ja` が既定、日本語先のrepositoryは `ja-en`、検査しないなら `off`）。英語側は簡潔に書き、長い補足が必要な場合だけ本文へ日本語で書く。
