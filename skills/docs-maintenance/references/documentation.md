...
<!-- textlint-enable prh -->
- 文体・用語の機械検査はtextlint（pre-commitで全リポジトリ既定実行。リポジトリ直下の設定がkit同梱規則より優先）が担い、意味の検査（指示対象の固定・メリハリ・読者前提・理由の欠落）は公開・提出前の `DOCS_REVIEW_PROMPT.md` レビューが担う。同型の用語指摘が2件以上出たら、kitの `prh.yaml` へ登録して機械検査化する。
