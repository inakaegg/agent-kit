...
- `DOCS_LEVEL=minimal` のrepoでは、人間向け文書は骨組みか要点までとし、読みやすさレビューをしない（外部へ提出する文書は例外で、下の2項目に従う）。`DOCS_LEVEL` はagent-settingsのキーで既定値は `minimal`。単体配布では同梱の `references/model-defaults.env` が持ち、どこにも値が無い場合も `minimal` として扱う。公開または共有を始める前には、追跡対象の `agent-settings.env` に `DOCS_LEVEL=full` を置き、骨組みだけだった文書を完成文書へ整えてから、下の2項目のレビューを通す
