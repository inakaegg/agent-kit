...
- `DOCS_LEVEL=minimal`（agent-settings）のrepoでは、人間向け文書は骨組みか要点までとし、読みやすさレビューをしない（外部へ提出する文書は例外で、下の2項目に従う）。公開前に `full` へ戻して整える順序は `docs/policies/git-and-remote.md` に従う
