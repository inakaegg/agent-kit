...
- **重リスク作業**（公開API、永続化、並行・非同期、認証・security、課金、migration、deploy、広いarchitecture）：仕様レビュー → 実装計画レビュー → 実装レビュー。実装計画を書く前に仕様レビューを通し、3段階を統合しない。
...
- 確認済みの正しさ・security・データ消失・互換性・明示要件違反（合格条件の検証不能を含む）は修正必須としてすべて報告し、報告数・必須数の上限から除外する。それ以外は最大5件、読みやすさだけの修正必須はうち2件までとする。件数だけで重大指摘を省略・格下げしない
...
- `VERDICT: LGTM` が出たら通常通過として終了する。未完了レビューは例外通過もできない。
- 2回を既定とする。2回目は前回の修正必須の解消と修正差分を確認する。新たな正しさ・security・データ消失・互換性・明示要件違反の修正必須は、回数だけでRECORDにしない。それ以外の新規指摘はRECORDへ置く。
- コード変更を含まない文書・計画で、全修正必須の事実を実データで裏取りして修正し、該当検査と修正差分点検を済ませた場合は、再レビューを省略し「自己検証による例外通過」としてよい。security・データ消失・互換性破壊の指摘は省略不可。通常のコード修正は新しい独立セッションで再レビューする。
- 2回終了後に正しさ・security・データ消失・互換性・明示要件違反の修正必須が残れば、修正・再検査後の独立した解消確認を1回だけ追加する。LGTMが得られなければ停止して人間判断へ戻す。重大な修正必須がなく軽微な残指摘だけなら、課題へ残して「記録付き例外通過」とする。
- 公開・提出する人間向け文書の読みやすさレビューは例外通過の対象外とし、LGTMを得る。重大な修正必須、または2回目で新たな初見読者の目的不達があれば、修正後の解消確認を計1回だけ追加できる。なおLGTMが得られなければ停止する。
- 同じ修正必須が新しい証拠なく反復する場合は、回数を重ねず人間判断へ戻す。
- 各回で独立性と§3の件数規則を守る。同型の指摘が2件以上なら、対応表で監査し検出可能な部分を機械検査にする。
- full kitでは `docs/policies/review.md` の「通過条件」を正本とする。単体配布では本節で判断する。実在するverdictと通過理由、検査証跡、残件を分けて報告し、例外通過をLGTMと書かない。
- 別系統が利用不能な場合の「同系統による暫定通過」は通過理由とは別に記録し、復旧後にその段階をやり直す。`AUTO_MERGE_PRIVATE` による許可なしmergeは、必要なレビューすべてのLGTMと暫定通過のやり直し完了を要し、自己検証・記録付き例外通過では行わない。
- この上限はPR bot・CI反復（`$pr-review-loop`）とは別に数える。
