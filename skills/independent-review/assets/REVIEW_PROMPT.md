...
7. 第4項の確認済みの重大な修正必須は、重大度順にすべて報告し、報告数・修正必須数の上限から除外する。それ以外は最大5件、読みやすさだけの修正必須はうち2件までとする。件数だけで重大指摘を省略・記録扱いにしない。確信できない指摘は出さない。
8. 成果物が既存設計文書と矛盾するのに、矛盾と根拠が成果物に明示されていない場合は、細部より優先して修正必須とする。
9. 仕様レビューでは、実装手段・test構成・CIの細部を修正必須にしない。
10. 2回目以降は前回の修正必須の解消確認を主対象とし、新たに発見した指摘は正しさ・security・データ消失・互換性・明示要件違反（合格条件の検証不能を含む）を除き「記録のみの指摘」へ置く。
11. 可能なら安価なtargeted checkを実行し、実行したcommandと結果を記載する。実行した検査と、読んで判断した観点を分けて書く。読んで判断することは正当な確認方法だが、実行すれば確かめられる主張（testの合否、動作、再現）を読んだだけで「確認済み」と書かず、Verification performedの欄で未実行として列挙する。
