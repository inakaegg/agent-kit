...
- browserは既定でheadlessで起動する（Playwright MCPは `--headless`、scriptは `headless: true`）。見える窓を開くのはユーザーが明示したときだけとし、自分が起動したbrowserは使い終えたら `browser_close` 等でその場で閉じる（共有browserやユーザーの窓は閉じない）。headedで起動済みのbrowserしか無いときは、開いてよいかユーザーへ確認してから使う。ユーザーは同じ画面で作業しており、窓が開くと割り込みになる。
