...
- browserは既定でheadlessで起動する（Playwright MCPは `--headless`、scriptは `headless: true`）。見える窓は使用中のデスクトップへの割り込みになり得るため、開くのはユーザーが明示したときだけとする。自分が起動したbrowserは使い終えたら `browser_close` 等でその場で閉じる（共有browserやユーザーの窓は閉じない）。headedで起動済みのbrowserしか無いときは、開く前にユーザーへ確認する。応答を得られない環境（CI、専用の検証機など）ではそのまま使い、報告へその旨を書く。
