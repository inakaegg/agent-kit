...
- `pre-commit` also checks relative links in staged `.md` files and rejects the commit when a link target does not exist. It is a deterministic file-existence check (no LLM, a few milliseconds); external URLs, in-page anchors, absolute paths, and links inside code fences are ignored. Where perl is not available the check is skipped. To opt a repository out, set `git config --local hooks.skipLinkCheck true`.
