...
The hooks stand down in one situation: a repository under the system temp directory (`$TMPDIR`, `/tmp`, `/var/tmp`, and on macOS the per-user `/private/var/folders`) that reaches them only through the global `core.hooksPath`. Test suites of other projects create throwaway repositories there and commit into them, and the main-branch guard would otherwise fail those tests for committing code on `main`. The stand-down covers all three hooks, secrets scan included; the repository's own `.git/hooks/*` still run, because with a global `core.hooksPath` the kit's hooks are the only path to them. A repository that sets `core.hooksPath` in its own `.git/config` is never skipped — that is how the kit's own hook tests keep running, and how you keep the guards while deliberately working under a temp directory. A `$TMPDIR` that points at `/` or at an ancestor of `$HOME` is ignored; a `$TMPDIR` inside `$HOME` is honoured, so keep it pointed at a real scratch directory.
...
Three rules in `AGENTS.md` have no enforcement point on the git side: the ban on `--no-verify` (a hook cannot prevent its own bypass), the ban on `git add .` / `-A` / `--all` (git has no hook for staging), and the rule that creating a remote repository, pushing a local repository that has no remote yet, and making an existing repository public are done by the user, never by the agent. `scripts/git-guard-hook.py` closes all three by inspecting each shell command the agent is about to run and blocking the forbidden forms before they execute: `gh repo create` / `new` / `fork`, `gh repo edit --visibility public`, `gh api` writes of `visibility=public` / `private=false` or `--input` updates of a repository, `git push` and `git remote add` from a repository that has no remote (resolved through `cd` and `git -C`), and `git push` to a literal URL. A `git push -u` of a new branch to an existing remote is ordinary PR work and passes, and the guard looks at command positions rather than quoted text, so mentioning these commands in a heredoc or a `grep` does not trip it. When the user has directly instructed one remote-creation operation, the agent prefixes the command with `AGENT_USER_DIRECTED=1`; the prefix stays visible in the transcript as the reason the guard let it through, and it covers exactly one such operation per command. It binds only the agent's tool calls; a human typing the same commands in a terminal is unaffected. Both CLIs report the shell tool as `Bash` with the command in `tool_input.command`, so the registration is the same block in either settings file (`~/.claude/settings.json`, or `~/.codex/hooks.json` for Codex, which then needs the `/hooks` trust step above):
