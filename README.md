...
Three rules in `AGENTS.md` have no enforcement point on the git side: the ban on `--no-verify` (a hook cannot prevent its own bypass), the ban on `git add .` / `-A` / `--all` (git has no hook for staging), and the rule that creating a remote repository, pushing a local repository that has no remote yet (`git push -u` from such a repository, `git remote add`), and changing a repository's visibility (`gh repo edit --visibility`, `gh api` writes to `visibility` / `private`) are done by the user, never by the agent. A `git push -u` of a new branch to an existing remote is ordinary PR work and passes. `scripts/git-guard-hook.py` closes all three by inspecting each shell command the agent is about to run and blocking the forbidden forms before they execute. When the user has directly instructed one of the remote-creation operations, the agent prefixes the command with `AGENT_USER_DIRECTED=1`; the prefix stays visible in the transcript as the reason the guard let it through. It binds only the agent's tool calls; a human typing the same commands in a terminal is unaffected. Both CLIs report the shell tool as `Bash` with the command in `tool_input.command`, so the registration is the same block in either settings file (`~/.claude/settings.json`, or `~/.codex/hooks.json` for Codex, which then needs the `/hooks` trust step above):
